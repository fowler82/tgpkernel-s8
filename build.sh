...
mv -f $WORKDIR/ramdisk/image-new.img $ZIPDIR/tgpkernel/kernels/g530x.img
mv -f $OUTPUT/build.log $OUTPUT/build-g950x.log
ZIP_DATE=`date +%Y%m%d`
...
mv -f $WORKDIR/ramdisk/image-new.img $ZIPDIR/tgpkernel/kernels/g955x.img
mv -f $OUTPUT/build.log $OUTPUT/build-g955x.log
ZIP_DATE=`date +%Y%m%d`
...
mv -f $WORKDIR/ramdisk/image-new.img $ZIPDIR/tgpkernel/kernels/g950x.img
mv -f $OUTPUT/build.log $OUTPUT/build-g950x.log
ZIP_DATE=`date +%Y%m%d`
ZIP_NAME=$KERNELNAME.G950x.$ANDROIDVERSION.v$VERSION_NUMBER.$ZIP_DATE.zip
ZIP_FILE_TARGET=$ZIPDIR/$ZIP_NAME
